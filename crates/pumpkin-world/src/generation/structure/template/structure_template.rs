...
/// A block captured from a live world before it is interned into a template palette.
#[derive(Debug, Clone)]
...
impl StructureTemplate {
    /// Builds a template from captured world blocks using vanilla's block ordering and palette
    /// interning rules.
    #[must_use]
    pub fn from_captured(
        size: Vector3<i32>,
        mut captured: Vec<CapturedBlock>,
        entities: Vec<TemplateEntity>,
    ) -> Self {
        captured.sort_by_key(|block| (block.pos.y, block.pos.x, block.pos.z));
        let (mut with_nbt, without_nbt): (Vec<_>, Vec<_>) =
            captured.into_iter().partition(|block| block.nbt.is_some());
        let (mut full, mut other): (Vec<_>, Vec<_>) =
            without_nbt.into_iter().partition(|block| block.full_cube);
        ...
        let blocks = ordered
            .into_iter()
            .map(|block| StructureBlockInfo::new(block.pos, block.palette_entry, block.nbt))
            .collect();
        let entity_info_list = entities
            .into_iter()
            .map(|entity| StructureEntityInfo::new(entity.pos, entity.block_pos, entity.nbt))
            .collect();
        let mut template = Self {
            palettes: vec![Palette::new(blocks)],
            entity_info_list,
            size,
            author: String::new(),
            palette: Vec::new(),
    ...
    /// Serializes this template to gzip-compressed NBT bytes.
    pub fn to_nbt_bytes(&self) -> Result<Vec<u8>, TemplateError> {
        Ok(write_gzip_compound_tag_to_bytes(self.save())?)
    }

    /// Writes this template to a gzip-compressed NBT file, creating parent directories as
    /// needed.
    pub fn save_to_path(&self, path: &Path) -> Result<(), TemplateError> {
        ...
        write_gzip_compound_tag(self.save(), file)?;
        Ok(())
    }

    /// Loads a template from a gzip-compressed NBT file.
