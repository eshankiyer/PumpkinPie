...
impl GenerationSchedule {
    ...
    #[expect(clippy::too_many_lines)]
    /// `dim_sections` is the dimension's full section count (`dimension.height / 16`), which is
    /// what the client reads from the chunk-data packet. Chunks arriving here can be shorter than
    ...
    fn receive_chunk(
        ...
        match data {
            RecvChunk::IO(chunk) => {
                ...
                let Some(mut holder) = self.chunk_map.remove(&pos) else {
                    warn!("receive_chunk(IO): missing holder for {:?}", pos);
                    return;
                };
    ...
    fn work(mut self, level: &Arc<Level>) {
        ...
        let dim_sections = level.world_gen.load().dimension().height as usize / 16;
        ...
        let dim_sky_light = u8::from(level.world_gen.load().dimension().has_skylight) * 15;
        debug!(
