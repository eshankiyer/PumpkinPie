...
fn broadcast_swing(target: &dyn crate::entity::EntityBase, hand: Hand) {
...
fn swing_targets(
    ...
    for target in targets {
        if target.get_living_entity().is_some() {
            broadcast_swing(target.as_ref(), hand);
            swung += 1;
        }
    }
...
impl CommandExecutor for SelfExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        swing_targets(context, &[target], Hand::Right)
    }
}
...
impl CommandExecutor for TargetsMainHandExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        ...
        swing_targets(context, &targets, Hand::Right)
    }
}
...
impl CommandExecutor for TargetsOffHandExecutor {
    fn execute(&self, context: &CommandContext) -> CommandExecutorResult {
        let targets = EntityArgumentType::get_entities(context, ARG_TARGETS)?;
        swing_targets(context, &targets, Hand::Left)
    }
}
