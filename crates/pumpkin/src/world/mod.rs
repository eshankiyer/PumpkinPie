...
/// Ray/box intersection returning the hit face and ray parameter `t`.
type RayBoxHit =
    fn(Vector3<f64>, Vector3<f64>, Vector3<f64>, Vector3<f64>) -> Option<(BlockDirection, f64)>;
use pumpkin_protocol::{
...
/// same world tick.
fn run_chunk_tick_phase<T, F>(ticks: impl IntoIterator<Item = T>, mut run: F)
where
    F: FnMut(T),
{
    for tick in ticks {
...
pub struct World {
    ...
    pub scoreboard: Arc<std::sync::Mutex<Scoreboard>>,
...
impl World {
    ...
    pub fn load(
        ...
        scoreboard: Arc<std::sync::Mutex<Scoreboard>>,
        dimension: Dimension,
    ...
    pub fn register_game_event_listener(&self, listener: Arc<dyn game_event::GameEventListener>) {
    ...
    pub fn unregister_game_event_listener_at(&self, pos: &BlockPos) {
        self.game_event_listeners.lock().unwrap_or_else(std::sync::PoisonError::into_inner).retain(|listener| {
    ...
    pub fn unregister_game_event_listener_for_entity(&self, uuid: uuid::Uuid) {
        self.game_event_listeners.lock().unwrap_or_else(std::sync::PoisonError::into_inner).retain(|listener| {
    ...
    pub fn tick(self: &Arc<Self>, server: &Arc<Server>) {
        ...
        // `ServerLevel.tick` runs `raids.tick` after the scheduled ticks and before the chunk
        // source and entities.
        raid::RaidManager::tick(&self.raids, self);
        ...
        tickable
            ...
            .for_each(|batch| {
                ...
                for entity in batch {
                    // Vanilla `ServerLevel.tick` runs `checkDespawn` for every live
                    // entity before the ticking-range check below.
                    ...
                    // A chunk stays active while it is still being generated. Mobs
                    // spawned by the generator are added to the world before their chunk
                    // is published, and every block read in a missing chunk reports air,
                    // so ticking them here would let them fall through the terrain that
                    // is about to appear.
        ...
        block_entities.par_chunks(16).for_each(|batch| {
            ...
            for be in batch {
                ...
                if be.is_dirty() {
                    ...
                    w_clone.update_comparators(&position, changed_block);
                    be.clear_dirty();
                }
            }
        });
    ...
    pub fn tick_environment(self: &Arc<Self>) {
        ...
        self.sky_darken.store(
            Self::sky_darken_for_time(time_of_day, rain_level, thunder_level),
            Relaxed,
        );
    }
    ...
    pub fn tick_chunks(self: &Arc<Self>, server: &Arc<Server>) {
        ...
        for chunk_pos in active_chunks.iter().copied() {
            if rng().random_range(0..48) == 0 {
                self.tick_precipitation_chunk(chunk_pos);
            }
        }

        // Vanilla executes these three phases serially. Only the independent natural-spawn work
        // below runs in parallel, after all tick-induced block and fluid changes settle.
        ...
        let spawn_passives = self
            .level_time
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .opens_persistent_spawn_gate();
        ...
        if spawn_mobs {
            if spawn_enemies {
                custom_spawners::tick_phantom_spawner(self);
                custom_spawners::tick_village_siege(self, spawn_enemies);
            }
            custom_spawners::tick_cat_spawner(self);
            custom_spawners::tick_wandering_trader_spawner(self);
        }
        ...
        if !spawn_list.is_empty() {
            ...
            let spawn_handle = server.runtime.clone();
    ...
    fn tick_precipitation_chunk(self: &Arc<Self>, chunk_pos: Vector2<i32>) {
        ...
        if let Some(precipitation) = precipitation {
            ...
            if let Some(pumpkin_block) = self.block_registry.get_pumpkin_block(below_block.id) {
                pumpkin_block.handle_precipitation(HandlePrecipitationArgs {
                    world: self,
                    block: below_block,
    ...
    #[must_use]
    pub fn is_raid_pre_raid_at(&self, position: BlockPos) -> bool {
        self.raids
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .is_pre_raid_at(position)
    }
    ...
    pub fn is_raining_at(&self, pos: &BlockPos) -> bool {
        self.is_raining() && self.is_raining_at_unchecked(pos)
    }
    ...
    pub async fn spawn_java_player(
        ...
        let last_death_location = player.get_last_death_location();
        debug!(
        ...
        let mut players_tab_list_names = Vec::new();
        {
            ...
            for (properties, player) in &data_to_process {
                // Resolve the async tab-list name before taking the (sync) chat-session lock so
    ...
    pub fn explode_with_calculator_from(
        self: &Arc<Self>,
        position: Vector3<f64>,
        power: f32,
        interaction: ExplosionInteraction,
    ...
    pub fn explode_with_fire_and_calculator(
        self: &Arc<Self>,
        position: Vector3<f64>,
        power: f32,
        interaction: ExplosionInteraction,
    ...
    pub fn explode_knockback_only(
        self: &Arc<Self>,
        position: Vector3<f64>,
        radius: f64,
        knockback_multiplier: f32,
    ) {
        Explosion::new_knockback_only(position, radius, knockback_multiplier).explode(self);
    ...
    pub async fn respawn_player(self: &Arc<Self>, player: &Arc<Player>, alive: bool) {
            ...
            player.calculate_respawn_point().await
        {
            ...
            // `TeleportTransition.missingRespawnBlock` / `createDefault`: fall back to the
            // shared world spawn in the respawn dimension (`findRespawnDimension`, the
            // overworld), adjusted to a safe position (`findAdjustedSharedSpawnPos`).
                ...
                if first_load {
                    ...
                    for entity_nbt in &entity_nbts {
                        ...
                        if entity.broadcast_to_player(&player) {
                            ...
                            entity.send_pairing_data_to(player.as_ref());
    ...
    pub fn set_block_state_if_validated(
        self: &Arc<Self>,
        position: &BlockPos,
        expected_state: BlockStateId,
        block_state_id: BlockStateId,
    ...
    fn set_block_state_internal(
        ...
        let mut unsent_block_changes = self
            .unsent_block_changes
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
            ...
            && let Some(entity) = self.get_block_entity(position)
        {
            ...
            // `BlockEntity::on_block_replaced` still returns a future; its implementations do not
            // await anything, so drive it to completion inline to keep vanilla's ordering
            // (`WorldChunk.java` 305-314: the entity reacts before it is removed).
            futures::executor::block_on(entity.on_block_replaced(self.clone(), *position));
            if !is_current_mutation() {
                clear_mutation_version();
                return None;
            }
        ...
        if is_current_mutation() {
            ...
            if flags.contains(BlockFlags::NOTIFY_NEIGHBORS) {
                ...
                if crate::block::has_analog_output_signal(new_block) {
                    self.update_comparators(position, new_block);
                    if !is_current_mutation() {
                        clear_mutation_version();
                        return None;
                    }
    ...
    pub fn is_close_to_village(&self, pos: BlockPos, section_distance: i32) -> bool {
        ...
        self.sections_to_village(pos) <= section_distance
    }
    ...
    pub fn village_poi_positions_in_range(&self, center: BlockPos, radius: i32) -> Vec<BlockPos> {
        let mut storage = self
            .portal_poi
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn acquire_poi(&self, poi_type: &str, center: BlockPos, radius: i32) -> Option<BlockPos> {
        let mut storage = self
            .portal_poi
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn release_poi(&self, pos: BlockPos) -> bool {
        let mut storage = self
            .portal_poi
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub fn is_redstone_torch_toggled_too_frequently(
        &self,
        pos: BlockPos,
        now: i64,
        add: bool,
    ...
    pub fn prune_redstone_torch_toggles(&self, now: i64) {
        self.redstone_torch_toggles
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .prune(now);
    ...
    pub fn break_block(
        ...
        if let Some(server) = self.server.upgrade() {
            server
                .plugin_manager
                .fire_blocking::<BlockBreakEvent>(&server, &mut event);
        }
        ...
        if !event.cancelled {
            ...
            self.set_block_state(position, new_state_id, flags);
            ...
            if Block::from_state_id(broken_block_state) != &Block::FIRE {
                ...
                match &cause {
                    Some(player) => {
                        if let ClientPlatform::Bedrock(client) = player.client.as_ref()
                            && let Ok(packet_data) = client.serialize_packet(&be_particles_packet)
                        {
                            client.try_enqueue_packet(packet_data);
                        }
                        self.broadcast_to_chunk_except_editioned(
                    ...
                    None => self.broadcast_to_chunk_editioned(
                        chunk_pos,
                        &je_particles_packet,
                        &be_particles_packet,
                    ),
            ...
            if !flags.contains(BlockFlags::SKIP_DROPS) {
                ...
                let is_raining = self.is_raining();
                let is_thundering = self.is_thundering();
    ...
    pub fn update_comparators(self: &Arc<Self>, position: &BlockPos, changed_block: &Block) {
        for direction in BlockDirection::horizontal() {
            ...
            if let Some(target) = comparator_notification_target(
                ...
                self.update_neighbor(&target, changed_block);
            }
        }
    }
    ...
    pub fn update_from_neighbor_shapes_vanilla(
        self: &Arc<Self>,
        state_id: BlockStateId,
        pos: &BlockPos,
    ) -> BlockStateId {
    ...
    /// Slab test shared by the ray/box helpers below. Returns the entry face and ray
    /// parameter `t` in `[0, 1]` of the first crossing, or `(None, 0.0)` when `from` already
    /// lies inside the box.
    fn clip_aabb(
        from: Vector3<f64>,
        to: Vector3<f64>,
        min: Vector3<f64>,
        max: Vector3<f64>,
    ) -> Option<(Option<BlockDirection>, f64)> {
        let dir = to.sub(&from);
        ...
        let face = match (hit_axis, hit_is_min) {
    ...
    /// `AABB.clip`: only a ray entering the box through one of its faces hits it; a ray
    /// that starts inside the box does not.
    fn intersects_aabb_with_direction(
        ...
        let (face, t) = Self::clip_aabb(from, to, min, max)?;
        face.map(|face| (face, t))
    }

    /// Ray/box hit as `(t, face, hit position)`. Unlike [`Self::intersects_aabb_with_direction`],
    /// a ray starting inside the box hits it at `from`, facing against the ray.
    #[must_use]
    pub fn intersects_aabb_with_hit(
        ...
        let (face, tmin) = Self::clip_aabb(from, to, min, max)?;
        let dir = to.sub(&from);

        let direction = face.unwrap_or(if dir.y < 0.0 {
            BlockDirection::Up
        } else if dir.y > 0.0 {
            BlockDirection::Down
        } else {
    ...
    /// [`Self::intersects_aabb_with_hit`] reduced to `(face, t)`.
    fn intersects_aabb_including_start(
        from: Vector3<f64>,
        to: Vector3<f64>,
        min: Vector3<f64>,
        max: Vector3<f64>,
    ) -> Option<(BlockDirection, f64)> {
        Self::intersects_aabb_with_hit(from, to, min, max).map(|(t, direction, _)| (direction, t))
    }

    /// Closest hit of the ray against the block's (non-empty) outline shape, as
    /// `(face, t)`, with `box_hit` deciding how a single box is hit.
    fn ray_outline_hit(
        &self,
        block_pos: &BlockPos,
        from: Vector3<f64>,
        ...
        for shape in bounding_boxes {
            ...
            let Some((direction, distance)) = box_hit(from, to, world_min, world_max) else {
                continue;
            };
        ...
        let (outline_direction, outline_distance) = outline_hit?;
        ...
        if Block::from_state_id(state.id) == &Block::HOPPER {
            ...
            for shape in crate::block::blocks::hopper::interaction_shapes_at(state.id, block_pos) {
                if let Some((direction, distance)) = box_hit(from, to, shape.min, shape.max)
                    && interaction_hit
            ...
            {
                return Some((direction, outline_distance));
            }
        }
        ...
        if Block::from_state_id(state.id) == &Block::COMPOSTER {
            ...
            if let Some((interaction_direction, interaction_distance)) =
                box_hit(from, to, block_min, block_max)
                && interaction_distance < outline_distance
            {
                return Some((interaction_direction, outline_distance));
            }
        }
        ...
        if matches!(
            ...
            if let Some((interaction_direction, interaction_distance)) =
                box_hit(from, to, shape.min, shape.max)
                && interaction_distance < outline_distance
            {
                return Some((interaction_direction, outline_distance));
            }
        ...
        Some((outline_direction, outline_distance))
    }

    /// Hit face and position of the ray against the block's outline (a full cube when the
    /// block has no outline shape). A ray starting inside the shape hits it at `from`.
    pub fn ray_outline_check_detailed(
        ...
        let (direction, t) =
            self.ray_outline_hit(block_pos, from, to, Self::intersects_aabb_including_start)?;
        Some((direction, from + to.sub(&from) * t))
    }
    ...
    fn ray_outline_check(
        ...
        self.ray_outline_hit(block_pos, from, to, Self::intersects_aabb_with_direction)
...
mod tests {
    ...
    #[tokio::test]
    fn chunk_tick_phases_complete_in_collection_order() {
