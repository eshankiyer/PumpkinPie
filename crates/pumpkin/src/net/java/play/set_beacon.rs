...
impl JavaClient {
    pub async fn handle_set_beacon(&self, player: &Arc<Player>, packet: &SSetBeacon) {
        ...
        let primary_effect =
            primary_id.and_then(|id| pumpkin_data::effect::StatusEffect::from_id(id as u16));
        let secondary_effect =
            secondary_id.and_then(|id| pumpkin_data::effect::StatusEffect::from_id(id as u16));

        if !BeaconBlockEntity::validate_effects(primary_effect, secondary_effect, levels) {
            warn!(
