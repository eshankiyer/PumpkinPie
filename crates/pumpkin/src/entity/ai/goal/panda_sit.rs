...
impl PandaSitGoal {
    ...
    fn edible_items(entity: &Entity, radius: f64) -> Vec<Arc<dyn EntityBase>> {
...
impl Goal for PandaSitGoal {
    fn can_start(&mut self, mob: &dyn Mob) -> bool {
        ...
        if !panda.held_stack().is_empty() {
            return true;
        }
        !Self::edible_items(&mob_entity.living_entity.entity, 6.0).is_empty()
    }
    ...
    fn start(&mut self, mob: &dyn Mob) {
        ...
        let mob_entity = panda.get_mob_entity();
        if panda.held_stack().is_empty() {
            let items = Self::edible_items(&mob_entity.living_entity.entity, 8.0);
    ...
    fn tick(&mut self, mob: &dyn Mob) {
        ...
        if !panda.is_sitting_panda() && !panda.held_stack().is_empty() {
            panda.try_to_sit();
        }
    }
    ...
    fn stop(&mut self, mob: &dyn Mob) {
        ...
        if !held.is_empty() {
            ...
            panda.set_held_stack(pumpkin_data::item_stack::ItemStack::EMPTY.clone());
