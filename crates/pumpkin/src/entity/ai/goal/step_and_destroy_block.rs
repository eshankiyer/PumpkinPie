...
pub trait Stepping: Send + Sync {
    ...
    fn tick_stepping_particles(&self, _world: Arc<World>, _block_pos: BlockPos) {}
    ...
    fn tick(&mut self, mob: &dyn Mob) {
        ...
        if counter > 0 {
            ...
            if let Some(stepping) = self.stepping.get() {
                stepping.tick_stepping_particles(world.clone(), tweak_pos);
            } else {
                self.tick_stepping_particles(world.clone(), tweak_pos);
            }
        }
