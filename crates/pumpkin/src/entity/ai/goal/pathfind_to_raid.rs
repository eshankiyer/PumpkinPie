...
fn raid_membership(mob: &dyn Mob) -> Option<i32> {
...
impl Goal for PathfindToRaidGoal {
    ...
    fn can_start(&mut self, mob: &dyn Mob) -> bool {
        ...
        if raid_membership(mob).is_none() {
            return false;
        }
    ...
    fn should_continue(&mut self, mob: &dyn Mob) -> bool {
        ...
        !entity.world.load().is_close_to_village(block_pos, 1)
    }
    ...
    fn tick(&mut self, mob: &dyn Mob) {
        let Some(raid_id) = raid_membership(mob) else {
            return;
        };
        let mob_entity = mob.get_mob_entity();
        ...
        if should_recruit {
            Self::recruit_nearby(&mob_entity.living_entity, &world, raid_id);
        }
...
impl PathfindToRaidGoal {
    ...
    /// current wave.
    fn recruit_nearby(recruiter: &LivingEntity, world: &std::sync::Arc<World>, raid_id: i32) {
