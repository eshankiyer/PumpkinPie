...
impl TransportItemsGoal {
    ...
    fn find_source(world: &World, origin: BlockPos) -> Option<BlockPos> {
    ...
    fn find_dest(world: &World, origin: BlockPos) -> Option<BlockPos> {
    ...
    fn take_one_item(inventory: &dyn Inventory) -> Option<ItemStack> {
        for i in 0..inventory.size() {
            let stack = inventory.get_stack(i);
            if !stack.is_empty() {
                return Some(inventory.remove_stack_specific(i, 1));
            }
        }
        None
    }

    fn deposit_item(inventory: &dyn Inventory, item: ItemStack) -> Option<ItemStack> {
        let mut remaining = item;
        for i in 0..inventory.size() {
            let mut stack = inventory.get_stack(i);
            if stack.is_empty() {
                inventory.set_stack(i, remaining);
                return None;
            }
    ...
    fn close_container(mob: &dyn Mob, inventory: &dyn Inventory) {
        inventory.on_close();
        if let Some(copper_golem) = Self::copper_golem(mob) {
...
impl Goal for TransportItemsGoal {
    fn can_start(&mut self, mob: &dyn Mob) -> bool {
        ...
        let Some(source) = Self::find_source(&world, origin) else {
            self.cooldown = mob.get_random().random_range(60..=100);
            return false;
        };
        if Self::find_dest(&world, origin).is_none() {
            self.cooldown = mob.get_random().random_range(60..=100);
            return false;
        }
    ...
    fn tick(&mut self, mob: &dyn Mob) {
        ...
        match self.phase {
            Phase::ToSource(source) => {
                ...
                let item = Self::take_one_item(source_inventory.as_ref());
                if item.is_none()
                    && let Some(copper_golem) = Self::copper_golem(mob)
                {
                    copper_golem.set_state(CopperGolemState::GettingNoItem);
                }
                Self::close_container(mob, source_inventory.as_ref());
                let Some(item) = item else {
                    self.phase = Phase::Idle;
                    self.cooldown = mob.get_random().random_range(60..=100);
                    return;
                };
                let Some(dest) = Self::find_dest(&world, origin) else {
                    if let Some(source_inventory) =
                        Self::open_container(mob, &world, source, CopperGolemState::DroppingItem)
                    {
                        let _ = Self::deposit_item(source_inventory.as_ref(), item);
                        Self::close_container(mob, source_inventory.as_ref());
                    }
                    self.phase = Phase::Idle;
                    return;
            ...
            Phase::ToDest { source, dest } => {
                ...
                {
                    let leftover = Self::deposit_item(destination_inventory.as_ref(), item);
                    if leftover.is_some()
                        && let Some(copper_golem) = Self::copper_golem(mob)
                    {
                        copper_golem.set_state(CopperGolemState::DroppingNoItem);
                    }
                    Self::close_container(mob, destination_inventory.as_ref());
                    ...
                    {
                        let _ = Self::deposit_item(source_inventory.as_ref(), leftover);
                        Self::close_container(mob, source_inventory.as_ref());
                    }
                }
                self.phase = Phase::Idle;
