...
impl WitherEntity {
    ...
    pub async fn perform_ranged_attack_at(&self, head: usize, target: &dyn EntityBase) {
    ...
    async fn update_bossbar(&self, world: &Arc<World>, progress: f32) {
    ...
    async fn remove_all_bossbar(&self, world: &Arc<World>) {
        let mut bossbar_players = self.bossbar_players.lock().await;
        let players = world.players.load();
        for player in players.iter() {
            if bossbar_players.contains(&player.gameprofile.id) {
    ...
    async fn tick_side_heads(&self, world: &Arc<World>, tick_count: i32) {
        let entity = &self.mob_entity.living_entity.entity;
        ...
        for i in 1..3 {
                ...
                && self.idle_head_updates[i - 1].fetch_add(1, Ordering::Relaxed) > 15
            {
                ...
                self.perform_ranged_attack(i + 1, xt, yt, zt, true).await;
            ...
            if head_target > 0 {
                ...
                if let Some(current) = current
                    ...
                    && entity
                        ...
                        <= 900.0
                    && self.mob_entity.has_line_of_sight(current.as_ref()).await
                {
                    self.perform_ranged_attack_at(i + 1, current.as_ref()).await;
                ...
                for candidate in world.get_entities_at_box(&search_box) {
                    ...
                    if living.is_valid_ai_target()
                        && predicate
                            .test(world, Some(&self.mob_entity.living_entity), living)
                            .await
                    {
    ...
    async fn tick_destroy_blocks(&self, world: &Arc<World>) {
    ...
    async fn ai_step_movement(&self) {
        let entity = &self.mob_entity.living_entity.entity;
    ...
    fn pre_damage<'a>(
        ...
        Box::pin(async move {
            if damage_type.has_tag(&tag::DamageType::MINECRAFT_WITHER_IMMUNE_TO)
                || source
                    .is_some_and(|source| source.get_entity().entity_type == &EntityType::WITHER)
    ...
    fn on_mob_death<'a>(&'a self, _cause: Option<&'a dyn EntityBase>) -> EntityBaseFuture<'a, ()> {
        Box::pin(async move {
            ...
            if world.level_info.load().game_rules.mob_drops {
                ...
                world.spawn_entity(nether_star).await;
            }
        })
    }
    ...
    fn mob_tick<'a>(&'a self, _caller: &'a Arc<dyn EntityBase>) -> EntityBaseFuture<'a, ()> {
        Box::pin(async move {
            ...
            if invulnerable > 0 {
                ...
                if next <= 0 {
                    world
                        .explode(entity.get_eye_pos(), 7.0, ExplosionInteraction::Mob)
                        .await;
                    if !entity.is_silent() {
            ...
            self.tick_side_heads(&world, age).await;
            ...
            self.tick_destroy_blocks(&world).await;
...
impl WitherRangedAttackGoal {
    ...
    async fn shoot(mob: &dyn Mob, target: &dyn EntityBase) {
        if let Some(wither) = mob.cast_any().downcast_ref::<WitherEntity>() {
            wither.perform_ranged_attack_at(0, target).await;
        }
    }
}
