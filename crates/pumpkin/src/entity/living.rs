...
impl LivingEntity {
    ...
    pub async fn add_effect(&self, mut effect: Effect) {
        ...
        if effect.effect_type == &StatusEffect::INSTANT_HEALTH {
            ...
            // Apply non-instant effects, merging with the running instance the way vanilla
            // `MobEffectInstance.update` does instead of overwriting it.
            let changed = {
                ...
                if !effects.is_empty() {
                    ...
                    for effect in effects.values() {
                        ...
                        if let Some(chain) = hidden_effects.get(effect.effect_type) {
                            let mut nested: Option<pumpkin_nbt::compound::NbtCompound> = None;
