...
impl DisplayEntity {
    ...
    pub async fn write_display_nbt(&self, nbt: &mut NbtCompound) {
        ...
        let brightness_override = self.brightness.load(Ordering::Relaxed);
    ...
    pub async fn read_display_nbt(&self, nbt: &NbtCompound) {
        ...
        if let Some(teleport_duration) = nbt.get_int("teleport_duration") {
            // Display.setTeleportDuration clamps to [0, 59].
            self.teleport_duration
                .store(teleport_duration.clamp(0, 59), Ordering::Relaxed);
        }
        // Brightness.pack/unpack in Display.java: block << 4 | sky << 20
