...
impl EntityBase for FireballEntity {
    ...
    fn on_hit(&self, hit: ProjectileHit) {
        ...
        if let ProjectileHit::Entity { ref entity, .. } = hit {
            ...
            let _ = entity.damage(self, 6.0, pumpkin_data::damage::DamageType::FIREBALL);
        }

        let hit_pos = hit.hit_pos();
        // `Fireball.onHit`: fire is placed only when mob griefing is on, which
        // `ExplosionInteraction::Mob` already resolves to a block-preserving blast.
        world.explode_with_fire(
            hit_pos,
