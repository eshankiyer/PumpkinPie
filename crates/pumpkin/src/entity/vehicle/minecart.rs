    ...
    pub fn comparator_output(&self) -> u8 {
        match &self.kind {
            ...
            MinecartKind::Chest(minecart) => {
                crate::block::calculate_comparator_output(minecart.inventory().as_ref())
            }
...
impl EntityBase for MinecartEntity {
    ...
    fn tick(&self, caller: &Arc<dyn EntityBase>, server: &Server) {
        ...
        if let MinecartKind::Spawner(minecart) = &self.kind {
            minecart.tick(&world, block_pos, &self.vehicle.entity);
        }
                ...
                if is_powered_rail {
                    ...
                    match &self.kind {
                        ...
                        MinecartKind::CommandBlock(minecart) => {
                            minecart.activate(&world, &self.vehicle.entity);
                        }
                        MinecartKind::Rideable(_) => {
                            let passengers = self
                                .vehicle
    ...
    fn init_data_tracker(&self) {
        ...
        if let MinecartKind::CommandBlock(minecart) = &self.kind {
            minecart.sync_metadata(&self.vehicle.entity);
        }
    }
    ...
    fn interact(&self, player: &Arc<Player>, item_stack: &mut ItemStack) -> bool {
        match &self.kind {
            MinecartKind::Chest(minecart) => minecart.interact(&self.vehicle.entity, player),
            MinecartKind::Furnace(minecart) => {
                minecart.interact(&self.vehicle.entity, player, item_stack)
            }
            MinecartKind::Hopper(minecart) => minecart.interact(&self.vehicle.entity, player),
    ...
    fn move_entity(&self, caller: &Arc<dyn EntityBase>, motion: Vector3<f64>) {
        ...
        let should_continue = self.push_entities(caller);
        if should_continue {
            let current_pos = self.vehicle.entity.pos.load();
            let back_motion = to_position.sub(&current_pos);
            self.vehicle.entity.move_entity(caller, back_motion);
        }
    }
