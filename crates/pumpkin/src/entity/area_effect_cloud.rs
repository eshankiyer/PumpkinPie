...
impl AreaEffectCloudEntity {
    ...
    fn set_radius(&self, radius: f32) -> f32 {
        let clamped = radius.clamp(0.0, MAX_RADIUS);
        *self
            .radius
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = clamped;
    ...
    pub fn set_potion_contents(&self, item_stack: ItemStack) {
        ...
        *self
            .potion_duration_scale
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = duration_scale;
        self.send_particle_metadata();
    }

    /// AreaEffectCloud.java:101-115 replaces the tracked particle option immediately.
    pub fn set_custom_particle(&self, particle: CustomParticle) {
        *self
            .custom_particle
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = Some(particle);
        self.send_particle_metadata();
    }
    ...
    fn send_particle_metadata(&self) {
        let custom_particle = self
            .custom_particle
            .lock()
...
impl EntityBase for AreaEffectCloudEntity {
    ...
    fn tick(&self, _caller: &Arc<dyn EntityBase>, _server: &Server) {
        ...
        let potion_duration_scale = *self
            .potion_duration_scale
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            * application_scale(0.0, radius.into());
        if effects.is_empty() {
            self.reapplication_map
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner)
