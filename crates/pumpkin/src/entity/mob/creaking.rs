...
impl CreakingEntity {
    ...
    pub fn activate(&self, player: &Arc<Player>) {
        *self
            ...
            .target
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner) = Some(player.clone());
        self.play_sound(Sound::EntityCreakingActivate);
        self.set_is_active(true);
        self.emit_entity_action();
    }

    fn emit_entity_action(&self) {
        let entity = &self.mob_entity.living_entity.entity;
...
impl Mob for CreakingEntity {
    ...
    fn pre_damage(&self, damage_type: DamageType, _source: Option<&dyn EntityBase>) -> bool {
        ...
        // `Creaking.hurtServer`: the gate is the `BYPASSES_INVULNERABILITY` damage-type
        // tag, not just out-of-world.
