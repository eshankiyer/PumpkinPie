...
impl FireBlock {
    ...
    fn try_spreading_fire(&self, world: &Arc<World>, pos: &BlockPos, chance: i32, age: u8) {
        ...
        if rand::rng().random_range(0..chance) < odds {
            ...
            if can_persist && !world.is_raining_at(pos) {
                ...
                if let Some(server) = world.server.upgrade() {
                    server
                        .plugin_manager
                        .fire_blocking(&server, &mut burn_event);
                }
                ...
                for yy in -1..=4 {
                    if xx != 0 || yy != 0 || zz != 0 {
                        ...
                        if ignite_odds > 0 {
                            ...
                            let can_ignite = odds > 0 && rand::rng().random_range(0..rate) <= odds;
                            if can_ignite && !Self::is_near_rain(world.as_ref(), &offset_pos) {
                                let spread_age =
