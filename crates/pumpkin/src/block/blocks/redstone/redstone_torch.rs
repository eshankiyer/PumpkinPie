...
impl BlockBehaviour for RedstoneTorchBlock {
    ...
    fn on_scheduled_tick(&self, args: OnScheduledTickArgs<'_>) {
        let state = args.world.get_block_state(args.position);
        let now = args.world.get_world_age();
        args.world.prune_redstone_torch_toggles(now);
        ...
        if args.block == &Block::REDSTONE_WALL_TORCH {
            ...
            let neighbor_signal = !should_be_lit(args.world, args.position, BlockDirection::Down);
            if handle_torch_tick(
                args.world,
                args.position,
                args.block,
...
fn handle_torch_tick(
    ...
    if *lit {
        if neighbor_signal {
            *lit = false;
            if world.is_redstone_torch_toggled_too_frequently(*pos, now, true) {
        ...
        false
    } else if !neighbor_signal && !world.is_redstone_torch_toggled_too_frequently(*pos, now, false)
    {
        *lit = true;
        true
    } else {
