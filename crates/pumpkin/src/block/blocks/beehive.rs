...
use crate::block::{
    BlockBehaviour, BlockMetadata, BrokenArgs, ExplodeArgs, GetComparatorOutputArgs, OnPlaceArgs,
...
impl BlockBehaviour for BeehiveBlock {
    ...
    fn player_will_destroy(&self, args: PlayerWillDestroyArgs<'_>) {
        ...
        if hive.is_empty() && props.honey_level == 0 {
            return;
        }
        ...
        let mut components = collect_components_from_block_entity(block_entity.as_ref());
        components.push((
    ...
    fn on_place(&self, args: OnPlaceArgs<'_>) -> BlockStateId {
        ...
        let item_stack = args.player.inventory().get_stack_in_hand(hand);
    ...
    fn use_with_item(&self, args: UseWithItemArgs<'_>) -> BlockActionResult {
        ...
        if item.id == Item::SHEARS.id {
            drop_honeycomb(args.world, args.position);
            args.world
            ...
            if !args.player.inventory().insert_stack_anywhere(&mut honey) {
                args.player.drop_item(honey);
            }
        } else {
            return BlockActionResult::PassToDefaultBlockAction;
        }
        ...
        reset_honey_level(args.world, args.block, args.position, state_id);
...
fn drop_honeycomb(world: &Arc<World>, position: &BlockPos) {
...
fn anger_nearby_bees(world: &Arc<World>, position: &BlockPos) {
        ...
        .into_values()
    {
        ...
        bee.mob_entity.set_target(Some(target));
    }
}
