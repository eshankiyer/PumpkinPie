...
use crate::block::{
    BlockBehaviour, BlockMetadata, GetComparatorOutputArgs, HandlePrecipitationArgs,
...
impl BlockBehaviour for CauldronBlock {
    fn on_entity_collision(&self, args: OnEntityCollisionArgs<'_>) {
        ...
        match args.block.id {
            BlockId::LAVA_CAULDRON => {
                if !entity.entity_type.fire_immune && !entity.fire_immune.load(Ordering::Relaxed) {
                    args.entity.set_on_fire_for(15.0);
                    args.entity.damage(args.entity, 4.0, DamageType::LAVA);
                }
            }
            BlockId::WATER_CAULDRON | BlockId::POWDER_SNOW_CAULDRON => {
                if entity.fire_ticks.load(Ordering::Relaxed) > 0 {
                    lower_fill_level(args.world, args.position, args.block);
                }
                entity.extinguish();
            }
            _ => {}
    ...
    fn handle_precipitation(&self, args: HandlePrecipitationArgs<'_>) {
...
fn lower_fill_level(world: &Arc<World>, position: &BlockPos, block: &Block) {
