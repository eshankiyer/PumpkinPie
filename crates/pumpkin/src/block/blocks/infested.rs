...
use crate::block::{BlockBehaviour, BlockMetadata, BrokenArgs, OnPlaceArgs};
...
impl BlockBehaviour for InfestedBlock {
    ...
    fn broken(&self, args: BrokenArgs<'_>) {
            ...
            tool.get_data_component::<pumpkin_data::data_component_impl::EnchantmentsImpl>()
                .is_some_and(|enchantments| {
                    enchantments.enchantment.iter().any(|(enchantment, _)| {
                        enchantment.has_tag(&tag::Enchantment::MINECRAFT_PREVENTS_INFESTED_SPAWNS)
                    })
                })
        };
        if prevents_spawn {
        ...
        args.world.spawn_entity(entity.clone());
