...
impl ScreenHandlerFactory for SmithingTableScreenFactory {
    fn create_screen_handler<'a>(
        ...
        Box::pin(async move {
            let handler: SharedScreenHandler = Arc::new(Mutex::new(SmithingScreenHandler::new(
                sync_id,
                player_inventory,
            )));
            Some(handler)
    ...
    fn get_display_name(&self) -> TextComponent {
        TextComponent::translate_cross(
            translation::java::CONTAINER_UPGRADE,
            translation::bedrock::CONTAINER_SMITHING_TABLE_UPGRADE,
            &[],
        )
    }
}
