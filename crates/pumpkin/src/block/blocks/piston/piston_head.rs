...
use crate::block::{
    BlockBehaviour, BrokenArgs, GetCloneItemStackArgs, OnNeighborUpdateArgs, OnStateReplacedArgs,
...
impl BlockBehaviour for PistonHeadBlock {
    ...
    fn broken(&self, args: BrokenArgs<'_>) {
        ...
        if Self::fitting_base(args.world, args.position, props) {
            args.world.break_block(&pos, None, BlockFlags::empty());
        }
    }
    fn on_neighbor_update(&self, args: OnNeighborUpdateArgs<'_>) {
        ...
        let base_survives = if base_block == &Block::PISTON || base_block == &Block::STICKY_PISTON {
        ...
        if base_survives {
            args.world.update_neighbor(&piston_pos, args.source_block);
        }
    }
