...
mod test {
    ...
    fn show_entity_hover_fields_are_inline() {
        ...
        // `HoverEvent.java:62-65` types `name` as `ComponentSerialization.CODEC`, whose
        // `Codec.either(Codec.either(Codec.STRING, ...), fullCodec)` shape
        // (`ComponentSerialization.java:107`) accepts a bare string for an unstyled
        // component. What matters here is that `name` is present and inline.
        assert!(
            hover.get_compound("name").is_some() || hover.get_string("name").is_some(),
            "name must be present as a component or a plain string"
        );
        assert!(
