...
impl CLevelChunk<'_> {
    pub fn encode_chunk(
        ...
        // `ChunkSections::section_count` is derived from the live section array, so it cannot
        // go stale after `ChunkData::pad_sections_to` grows the chunk.
        let sub_chunk_count = chunk.section.section_count() as u32;
