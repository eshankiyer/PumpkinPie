...
/// Retrieves the 2048-byte nibble array from a light container.
///
/// A uniform `Empty(d)` layer is real light data at level `d`, not an absent layer - Java's
/// `DataLayer.isEmpty()` is `data == null && defaultValue == 0` (`DataLayer.java:141-143`) -
/// so it serializes as `d` repeated, not as the caller's `default_val`. `default_val` applies
/// only when the section itself is missing.
#[must_use]
pub fn get_light_bytes(container: Option<&LightContainer>, default_val: u8) -> [u8; 2048] {
    let mut buf = [default_val << 4 | default_val; 2048];
    match container {
        Some(LightContainer::Full(data)) if data.len() == 2048 => buf.copy_from_slice(data),
        Some(LightContainer::Empty(default)) => buf.fill(default << 4 | default),
        _ => {}
    }
    buf
}
