    ...
    fn write_packet_data(
        ...
        if version >= &JavaMinecraftVersion::V_1_18 {
            ...
            if version >= &JavaMinecraftVersion::V_1_21_5 {
                ...
                // to the client, only persisted to disk. It is deliberately absent from
                // the compound built below.
...

            {
                ...
                // Shared with `CLightUpdate`: a uniform `Empty(15)` layer is real full data
                // on the wire (only an implicit zero layer is `DataLayer.isEmpty()`), so it
                // belongs in the data mask with a serialized 0xFF array.
